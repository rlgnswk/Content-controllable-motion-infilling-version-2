...
import models_blend_controllable_ver2_AE_z_regul as models
...
def main(args):
    ...
    for num_epoch in range(args.numEpoch):
        ...
        for iter, item in enumerate(train_dataloader):
            ...
            pred_affine, transition_latent = model(masked_input, blend_part_only)
            ...
            if iter % print_interval == 0 and iter != 0:
                ...
                log = "Train: [Epoch %d][Iter %d] [total_train_iter_loss(G): %.4f] [train_D_iter_loss: %.4f] [recon loss: %.4f] [G loss: %.4f] [train_z_iter_loss: %.4f]" %\
        ...
        for iter, item in enumerate(valid_dataloader):
            ...
            #kld_loss = torch.mean(-0.5 * torch.sum(1 + logvar.reshape(logvar.shape[0], logvar.shape[1], -1) - mean.reshape(mean.shape[0], mean.shape[1], -1) ** 2 - logvar.exp().reshape(logvar.shape[0], logvar.shape[1], -1), dim = 1), dim = 0).sum()
            kld_loss = 0
            total_v_loss = (recon_loss + loss_G + loss_Z_regularizer).item()
