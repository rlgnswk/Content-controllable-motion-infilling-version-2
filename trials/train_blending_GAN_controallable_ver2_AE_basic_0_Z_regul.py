    ...
    for num_epoch in range(args.numEpoch):
        ...
        for iter, item in enumerate(train_dataloader):
            ...
            if iter % print_interval == 0 and iter != 0:
                ...
                log = "Train: [Epoch %d][Iter %d] [total_train_iter_loss(G): %.4f] [train_D_iter_loss: %.4f] [recon loss: %.4f] [G loss: %.4f] [z_regul_loss: %.4f]" %\
        ...
        log = "Valid: [Epoch %d] [valid_epoch_loss(G): %.4f] [valid_epoch_D_loss: %.4f] [valid_epoch_recon_loss: %.4f] [valid_epoch_G_loss: %.4f] [valid_epoch_z_loss: %.4f]" %\
