...
class Style_Encoder_VAE(nn.Module):
        ...
        def forward(self, x):
            ...
            trans_latent = self.sampling(mean, logvar)
            recon_latent = self.sampling(torch.zeros_like(mean), torch.ones_like(logvar))
            
            return trans_latent.repeat(1,1,1,8), recon_latent.repeat(1,1,1,8), mean, logvar
...
class Convolutional_blend(nn.Module):
    ...
    def forward(self, masked_input, blend_part_only):
        ...
        return out_affine, out_recon, mean, logvar
